...
const long int MAX_CALLS = 100000000000; // 100 miliardi
int calls_performed;
...
int main(){ 
    ...
    // first, check if s is reachable from t
    reachability_check(G.size()-1);
    if(!reachable[0]){
        cout << "Source not reachable from target" << endl;
        return 0;
    }
