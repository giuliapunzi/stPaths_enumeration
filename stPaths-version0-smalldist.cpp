...
const long MAX_CALLS = 100000000000; 
int calls_performed;
...
void Dijkstra(int s){
    ...
    for(int i = 0; i<N; i++)// distance is infinity
        dist[i] = inf;

    dist[s] = 0;    

    vector<bool> Q(N, true); // initialize the queue
    while(find(Q.begin(), Q.end(), true) != Q.end()){ // while they are not all false = queue is not empty
        // find vertex in Q with min dist value
        ...
        // for(int u = minindex+1; u < N; u++){
        //     if(Q[u] && dist[u]<mindist){
                
        //         // cout << " Found: u= " << u << endl;
        //     }
        // }
        // cout << "Node of min distance is " << minindex << " with dist " << mindist << endl;

        // // if the node at minimum distance is t, we are done
        // if(minindex == t)
        //     return mindist;

        // otherwise, continue by removing minindex and iterating its neighbors
        Q[minindex] =false;

        for(auto v : G[minindex]){ // iterate neighbors of minindex that are in the queue
            if(Q[v]){
                int newd= dist[minindex] + 1;
                if(newd < dist[v] && dist[minindex] != inf){ // && dist[minindex] != inf?
                    dist[v] = newd;
                }
            }
        }
...
// builds a small graph containing only the nodes at max distance d(s,t)
// from either s or t
void extract_small(int s, int t){
    Dijkstra(s); // compute minimum distances from s
    int mindist = dist[t];
    vector<int> dist_s = dist; 

    Dijkstra(t);
    // at this point, dist_s contains the distances from s, while dist the ones from t
    // create a new graph with all nodes at distance at most mindist from either s or t
    int diffN=0;
    for(int u = 0; u < G.size(); u++){
        if(dist_s[u] > mindist && dist[u] > mindist){
            deleted[u] = true;
            diffN++;
        }
    }   

    cout << "Extracted graph has " << G.size()-diffN << " nodes; mindist=" << mindist << endl;
    return;
}
...
int main(){ 
    ...
    extract_small(0, G.size()-1); // extract small graph
    // extract_small(0,6);
    ...
    // here we also find the number of nodes and edges
    int numedges = 0;
    int numnodes = 0;
    ...
    for(int node = 0; node < G.size(); node++){
        if(!deleted[node]){
            numnodes++;
            numedges+= degree(node);
        }
        // numedges += G[node].size();
    }
    numedges = numedges/2;

    cout << "Graph has "<< numnodes << " nodes, " << numedges <<  " edges and maximum degree " << maxdeg << endl; 
    ...
    // reporting to file for small dist
    output_file.open("smalldist-comparison.txt", ios::app);
